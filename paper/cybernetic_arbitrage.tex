...
\midrule
Router learning leakage & Platform extracts/redistributes client data & $+0.207 \pm 0.014$ \\
Cross-node learning & Richer signal quality under ownership & $+0.065 \pm 0.011$ \\
Leakage protection & Lower information exposure for rollup & $+0.039 \pm 0.018$ \\
Actuator access & Rollup internal control advantage & $+0.005 \pm 0.002$ \\
Trust-generated context & Negligible & $+0.002 \pm 0.017$ \\
Context foreclosure & Noisy; graph-dependent & $-0.058 \pm 0.055$ \\
\midrule
\textbf{Total effect} & & $\mathbf{\sim +0.26}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Router learning leakage is the dominant channel} ($+0.207 \pm 0.014$), robust across all 5 seeds with the tightest standard deviation of any channel. If the router learning leakage hypothesis holds, it is the single largest driver of ownership advantage.

\textbf{Cross-node learning} ($+0.065 \pm 0.011$) and \textbf{leakage protection} ($+0.039 \pm 0.018$) are stable secondary channels. Together with router learning leakage, these three account for the bulk of the rollup's advantage.

\textbf{Context foreclosure is unstable} ($-0.058 \pm 0.055$). It was positive in one seed but negative in four---the noise added to external coordinators' context profiles can unpredictably help or hurt the rollup depending on graph topology. This channel, despite its microfounded motivation, does not reliably contribute to the rollup advantage in the current implementation.

\textbf{Trust and actuator are negligible.} The rollup wins through information channels (router leakage, cross-node learning, leakage protection) that the classical theory of the firm does not model.
