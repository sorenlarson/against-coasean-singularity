...
\noindent The logarithmic form of the revelation term ensures diminishing returns: the first units of context shared are most valuable. The remaining terms are: $\hat{\tau}_{ij}$, trust-generated context (Section~\ref{sec:trust}); $q$, the regime's policy quality; $\hat{a}_{ij}$, actuator access (Section~\ref{sec:actuator}); and $\phi_{ij}$, context fit:
