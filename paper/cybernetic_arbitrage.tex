...
\item \textbf{Trust and actuator access are real but favor the router.} Trust generates context that would not otherwise exist; actuator access determines whether coordination intent translates to execution. Both channels differ by institutional form, but they are the router's main differentiators versus the open market---the router invests in trust and has better actuator access than arm's-length transactions. Removing either channel from the model widens the rollup's advantage, because the router loses a strength while the rollup still has learning, leakage protection, and foreclosure. The rollup wins \textit{despite} the router's trust and actuator advantages, not because of its own.
\end{enumerate}
