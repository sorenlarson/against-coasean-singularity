...
\item \textbf{Counterparty selection and revelation.} For each opportunity, the node evaluates all its graph neighbors. For each candidate counterparty, it computes the optimal revelation level (grid search over $r \in \{0, 0.25, 0.5, 0.75, 1.0\}$) that maximizes net surplus given the current trust, leakage stock, policy quality, and institutional parameters. It selects the counterparty with the highest positive net surplus. If no counterparty yields positive surplus, the opportunity goes unexercised.
