...
\noindent Execution loss is the product of four factors: $\lambda_E$ scales the overall magnitude; \textbf{edge risk} $\epsilon_{ij}$ is the baseline probability of poor execution on this coordination relationship; \textbf{operational fragility} averages node $i$'s exception rate $\xi_i$ (how often non-standard cases arise) and node $j$'s quality risk $\omega_j$ (probability of operational failure); and the \textbf{control gap} $(1 - \hat{a}_{ij})$ means that higher actuator access directly reduces execution loss---an owner who can mandate process changes ($\hat{a} \approx 1$) loses almost nothing to execution failure, while a market participant who can only suggest ($\hat{a} \approx 0.25$) suffers the full risk.
