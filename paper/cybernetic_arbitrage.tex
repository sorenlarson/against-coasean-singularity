...
\item \textbf{Router learning leakage is the dominant channel} (55\% of Shapley-decomposed total effect). If the hypothesis that platform intermediation amplifies leakage holds, it is the single largest driver of ownership advantage---larger than cross-node learning, leakage protection, and context foreclosure combined.

\item In a \textbf{competing-world simulation} with a passive router (no strategic response to client loss), the parallel-world advantage compounds through competitive feedback: each acquisition strengthens the rollup while shrinking the router's client base, producing a tipping dynamic that drives the \textbf{router to extinction across all tested seeds}. The rollup achieves 3--4$\times$ the router's cumulative surplus.
...
Under the baseline calibration ($\lambda_L = 0.8$, $\gamma = 1.5$), match quality gains from revelation dominate leakage costs at every visibility level, so the optimizer selects $r = 1.0$ (full revelation) for all regimes. The revelation-leakage tradeoff is therefore \textit{theoretical} under the baseline: it describes the correct comparative statics but does not produce regime-differentiated revelation behavior. At higher leakage intensity ($\lambda_L \geq 2.0$), the tradeoff binds and regimes reveal different amounts---the rollup reveals more than the router, which reveals more than the market---consistent with Proposition~\ref{prop:tradeoff}. We retain the baseline calibration because the ownership advantage does not depend on differentiated revelation; it is driven by leakage costs, learning signal quality, and foreclosure, all of which operate at $r = 1.0$. A calibration where the tradeoff binds would add regime-differentiated revelation as an additional channel favoring the rollup.
\end{remark}
...
\midrule
Router learning leakage & Platform extracts/redistributes client data & $+0.200$ \\
Cross-node learning & Richer signal quality under ownership & $+0.068$ \\
Context foreclosure & Degraded context for external coordinators & $+0.047$ \\
Leakage protection & Lower information exposure for rollup & $+0.041$ \\
Actuator access & Rollup internal control advantage & $+0.008$ \\
Trust-generated context & Negligible & $+0.000$ \\
\midrule
\textbf{Total effect} & All-on (1.323) minus all-off (0.958) & $\mathbf{+0.365}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Router learning leakage is the dominant channel}, accounting for 55\% of the total effect (+0.200 of +0.365). If the router learning leakage hypothesis holds, it is the single largest driver of ownership advantage---larger than all other channels combined.

\textbf{Cross-node learning, foreclosure, and leakage protection} contribute +0.068, +0.047, and +0.041 respectively. Together with router learning leakage, these four information channels account for 97\% of the total effect. The rollup's advantage is fundamentally about information control.

\textbf{Trust is negligible} (+0.000) under the current calibration: the router's trust investment and the rollup's structural trust bonus roughly cancel out. \textbf{Actuator} is small (+0.008). The model is not rigged for the rollup---the two traditional ``ownership advantages'' (trust and control) contribute almost nothing. The rollup wins through information channels that the classical theory of the firm does not model.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_shapley.pdf}
\caption{Shapley value decomposition of the rollup/router surplus ratio. Each bar shows a channel's marginal contribution averaged over all possible orderings. Router learning leakage (the platform's systematic extraction of client data) accounts for 53\% of the total effect. Trust context is the only channel favoring the router.}
\label{fig:shapley}
\end{figure}
...
The parallel and competing results reinforce each other. The parallel model identifies \textit{which channels} create the structural edge (learning, leakage, foreclosure). The competing model shows \textit{what happens when that edge compounds}: not a permanent marginal advantage, but a tipping dynamic that eliminates the intermediary.
