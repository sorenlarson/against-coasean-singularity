...
\textbf{Context foreclosure is negligible} ($-0.004 \pm 0.001$). Despite its theoretical motivation---absorbing nodes should degrade competitors' information---the effect is small because the degraded context fit (one of seven inputs to the match quality sigmoid) produces only a tiny change in surplus per event. Per-interaction channels (router leakage, learning, leakage protection) dominate portfolio-level context degradation.
