...
\begin{equation}
j^* = \arg\max_{j \notin \mathcal{O}^t} \underbrace{c_1 \cdot \text{adj}(j)}_{\text{adjacency}} + \underbrace{c_2 \cdot \bar{\kappa}_j}_{\text{complementarity}} + \underbrace{c_3 \cdot \alpha_j}_{\text{info value}} + \underbrace{c_4 \cdot a_j}_{\text{actuator}}
\end{equation}

\noindent The rollup scores each unowned node on four criteria, weighted by coefficients $c_1 = 1.0$, $c_2 = 0.5$, $c_3 = 1.5$, $c_4 = 0.8$ (information value weighted highest, complementarity lowest):
\begin{itemize}[leftmargin=*]
