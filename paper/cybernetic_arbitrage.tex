...
\item \textbf{Ownership forecloses context from competitors.} When a node is acquired, its operational information---demand signals, capacity states, quality data---is withdrawn from the external information pool. External coordinators see increasingly degraded context as more of a vertical is absorbed.

\item \textbf{Trust and actuator access modulate coordination quality.} Trust generates context that would not otherwise exist; actuator access determines whether coordination intent translates to execution. These channels are available to all institutional forms---platforms can invest in trust, and partial actuator access is possible without ownership---but our simulation shows they primarily benefit the router rather than the rollup under the current calibration. The rollup's advantage comes from the first three channels.
\end{enumerate}
...
We characterize the revelation-leakage tradeoff analytically and identify sufficient conditions for rollup dominance under the model's parameter regime. We then investigate the model computationally through simulation with ablation analysis, multi-seed robustness, and a competing-world extension. Our main findings are:

\begin{enumerate}[leftmargin=*]
\item In parallel-world comparison across 10 graph realizations, the rollup generates a mean \textbf{25\% higher net surplus} than the router (10 of 10 seeds), driven largely by the router's learning leakage---the systematic extraction of client operational intelligence that benefits competitors.
...
\begin{itemize}
\item A vertical assignment $v_i \in \{1, \ldots, K\}$
\item Context vectors $\mathbf{d}_i, \mathbf{c}_i \in [0,1]^7$ (demand and capacity profiles)
\item Hidden alpha $\alpha_i \in [0,1]$ (private value of local information)
\item Price sensitivity $\pi_i \in [0,1]$
\item Actuator strength $a_i \in [0,1]$
\item Exception rate $\xi_i \in [0,1]$ and quality risk $\omega_i \in [0,1]$
\end{itemize}
\end{definition}
...
\begin{itemize}
\item Base surplus $s_{ij} > 0$ (potential value from coordination)
\item Leakage sensitivity $\ell_{ij} \in [0,1]$
\item Trust requirement $\tau_{ij} \in [0,1]$
\item Complementarity $\kappa_{ij} \in [0,1]$
\item Latency cost $\lambda_{ij} > 0$
\item Execution risk $\epsilon_{ij} \in [0,1]$
\end{itemize}
\end{definition}
...
\begin{itemize}
\item $\nu_{ij} \in [0,1]$ is \textit{market visibility}: the fraction of revealed context that leaks to the broader market
\item $\mu_{ij} \in [0,1]$ is \textit{actuator multiplier}: the degree of operational control over coordination execution
\item $\iota_{ij} \in [0,1]$ is \textit{interaction mode}: the quality of the interaction channel for trust generation
\end{itemize}
\end{definition}
...
Actuator $\mu_{ij}$ & 0.5 & 0.7 & $\begin{cases} 1.0 & i,j \in \mathcal{O}^t \\ 0.5 & \text{otherwise} \end{cases}$ \\[8pt]
...
Signal quality $\sigma_q$ & 0.3 & $0.5 + \text{breadth}$ & $\text{wtd}(\sigma_{\text{own}}, \sigma_{\text{mkt}})$ \\
Transfer bonus & 0 & 0 & $\tau \cdot |\{v_k : k \in \mathcal{O}^t\}| / K$ \\
...
Revelation improves coordination quality but with diminishing returns:

\begin{equation}
\label{eq:match}
m_{ij}(r) = \sigma\!\left(\beta_0 + \beta_1 \ln(1 + b_r \cdot r) + \beta_2 \cdot \hat{\tau}_{ij} + \beta_3 \cdot q + \beta_4 \cdot \hat{a}_{ij} + \beta_5 \cdot \kappa_{ij} + \beta_6 \cdot \phi_{ij}\right)
\end{equation}

\noindent where $\sigma(\cdot)$ is the sigmoid function, $\hat{\tau}_{ij}$ is trust-generated context (Section~\ref{sec:trust}), $q$ is the regime's policy quality, $\hat{a}_{ij}$ is the actuator term (Section~\ref{sec:actuator}), and $\phi_{ij}$ is context fit:
...
\begin{equation}
\label{eq:leakage}
L_{ij}(r) = \lambda_L \cdot r^{\gamma} \cdot \ell_{ij} \cdot \frac{\pi_i + \alpha_i}{2} \cdot \nu_{ij}
\end{equation}

\noindent with $\gamma > 1$ ensuring convexity (leaking a lot is disproportionately costly). The term $(\pi_i + \alpha_i)/2$ captures the value of the information being leaked: price sensitivity and hidden alpha.
...
\[
s_{ij} \cdot \sigma'(z) \cdot \frac{\beta_1 b_r}{1 + b_r r} = \lambda_L \gamma r^{\gamma-1} \ell_{ij} \frac{\pi_i + \alpha_i}{2} \nu_{ij}
\]
...
We implement the model as a discrete-time simulation over a directed graph with $N = 30$ nodes, $K = 5$ verticals, edge density 0.15, and $T = 50$ time steps. Node and edge parameters are drawn from uniform distributions. Three institutional forms are run on the same graph realization, differing only in parameter regime. All results are deterministic given a fixed random seed.
...
\begin{table}[H]
\centering
\caption{Ablation Analysis: Channel Decomposition (single-seed, seed 42, net of ownership costs)}
\label{tab:ablation}
\begin{tabular}{llcc}
\toprule
Channel Ablated & Description & Ratio & $\Delta$ from Baseline \\
\midrule
(none --- baseline) & All channels active & 1.103 & --- \\
Learning & No cross-node transfer & 1.044 & $-0.059$ \\
Leakage & Leakage cost set to zero & 1.071 & $-0.032$ \\
Complementarity & Exogenous complementarity enabled & 1.090 & $-0.013$ \\
Actuator & All regimes get market actuator & 1.117 & $+0.014$ \\
Trust context & Trust-generated context set to zero & 1.131 & $+0.028$ \\
All channels & All channels disabled & 0.985 & $-0.118$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Learning is the largest single channel.} Disabling cross-node learning reduces the ratio from 1.103 to 1.044---a drop of 0.059, the largest single-channel effect. Learning differences emerge from \textit{signal quality}---the rollup observes richer outcomes at owned nodes---not from an assumed learning rate advantage (all regimes share $\eta_{\text{base}} = 0.03$). The router has its own learning advantage: aggregate breadth across all client interactions. But depth plus cross-vertical transfer outweighs breadth.

\textbf{Leakage is second.} Disabling leakage reduces the ratio by 0.032. With context foreclosure included, the rollup's information advantage is robust even without explicit leakage costs.

\textbf{Sanity check passes.} With all channels disabled, the ratio falls to 0.985---the rollup loses because ownership costs remain but information advantages are gone. The 0.118 drop from baseline to all-off confirms the combined magnitude of the information channels.

\textbf{Trust and actuator channels favor the router.} A counterintuitive finding: removing trust-generated context or actuator access \textit{increases} the rollup/router ratio ($+0.028$ and $+0.014$ respectively). This occurs because the router depends on these channels more than the rollup does. The router's interaction mode ($\iota_R = 0.6$) combined with its trust investment mechanism generates substantial trust-based context---removing this channel hurts the router disproportionately. Similarly, the router's actuator multiplier ($\mu_R = 0.7$) gives it a meaningful advantage over the market ($\mu_M = 0.5$); equalizing actuator access removes this router advantage. The rollup's dominance comes from \textit{other} channels (learning, leakage, foreclosure) that more than compensate. These sign reversals confirm that the model is not rigged in the rollup's favor: two of its four conceptual pillars actually favor the router under the current parameterization.
...
Third, the baseline simulation uses 30-node graphs. We verified results at 200 nodes (10 seeds, mean ratio 1.23, 10/10 pass) and 1000 nodes (ratio 1.22, all propositions pass), confirming the mechanisms are scale-invariant. Richer graph topologies (clustering, hub-and-spoke) remain unexplored.
