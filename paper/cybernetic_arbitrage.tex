...
\midrule
Router learning leakage & Platform extracts/redistributes client data & $+0.198$ \\
Cross-node learning & Richer signal quality under ownership & $+0.077$ \\
Leakage protection & Lower information exposure for rollup & $+0.052$ \\
Context foreclosure & Degraded context for external coordinators & $+0.034$ \\
Trust-generated context & Favors the router & $-0.005$ \\
Actuator access & Favors the router & $-0.019$ \\
\midrule
\textbf{Total effect} & All-on (1.286) minus all-off (0.949) & $\mathbf{+0.337}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Router learning leakage is the dominant channel}, accounting for 59\% of the total effect. The router's practice of ingesting client operational data and using it to improve service for competitors imposes a cost that exceeds all other channels combined. This is the paper's central empirical finding: platform intermediation does not merely fail to shield information---it systematically extracts and redistributes it.

\textbf{Cross-node learning is second} at $+0.077$ (23\% of total). Learning differences emerge from signal quality, not assumed rates: the rollup observes richer outcomes at owned nodes while the router learns from aggregate breadth. Depth plus cross-vertical transfer outweighs breadth.

\textbf{Trust and actuator favor the router} ($-0.005$ and $-0.019$). The router must earn trust through investment to access context; the rollup accesses it through ownership. When trust is present, it narrows the gap. This confirms the model is not rigged: two channels actively favor the router, and the rollup wins despite them.
