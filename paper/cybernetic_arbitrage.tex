...
\item Hidden alpha $\alpha_i \in [0,1]$ --- private value of local information; higher alpha means the node has more competitively valuable context to protect (or leak)
\item Price sensitivity $\pi_i \in [0,1]$ --- how costly it is when the node's pricing information is exposed to competitors
...
\item Trust requirement $\tau_{ij} \in [0,1]$ --- how much trust is needed to surface valuable context on this edge
\item Complementarity $\kappa_{ij} \in [0,1]$ --- exogenous synergy between the two nodes (default coefficient zero; used only in robustness checks)
