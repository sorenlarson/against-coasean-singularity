...
\begin{abstract}
\begin{spacing}{1.2}
\noindent AI commoditizes execution, so value migrates to the inputs execution cannot replace: local context, trust, and operational control. A growing literature argues that AI agents will reduce transaction costs enough to shift activity from firms to markets---a ``Coasean singularity.'' This account treats coordination as an information-processing problem that AI makes cheap. We argue that coordination also requires \textit{context revelation}, which leaks the scarce resource that generates value in the first place. Who owns the assets where context is born determines who else benefits from it. We formalize this in a graph economy where three institutional forms---fragmented market, platform router, and vertically integrated rollup---coordinate over the same nodes and edges. We model the hypothesis that platform intermediation amplifies leakage: the router ingests clients' operational data and uses it to improve service for competitors, producing higher total leakage than the open market despite lower nominal visibility. In parallel-world comparison, the rollup generates a mean 24\% higher net surplus than the router across 10 seeds (10/10 pass). Under passive-router assumptions in a competing-world simulation, competitive feedback compounds this advantage into router extinction across all 10 tested seeds. Ownership of the scarce context, not intermediation of it, captures the value that AI creates.
...
\item \textbf{Ownership forecloses competitors' access.} In competitive dynamics, acquiring a node removes it from the router's client base, reducing the router's coordination opportunities and learning breadth. This foreclosure-through-node-removal is the mechanism that drives the competing-world tipping dynamic. Information degradation (external coordinators seeing blurred profiles of absorbed nodes) is theoretically motivated but empirically small in the parallel-world Shapley decomposition.
...
\begin{enumerate}[leftmargin=*]
\item In parallel-world comparison across 10 graph realizations, the rollup generates a mean \textbf{24\% higher net surplus} than the router (10 of 10 seeds), driven primarily by the router's learning leakage---the systematic extraction of client operational intelligence that benefits competitors.

\item \textbf{Router learning leakage is the dominant channel} (55\% of Shapley-decomposed total effect). If the hypothesis that platform intermediation amplifies leakage holds, it is the single largest driver of ownership advantage---larger than cross-node learning and leakage protection combined.

\item In a \textbf{competing-world simulation} with a passive router (no strategic response to client loss), the parallel-world advantage compounds through competitive feedback: each acquisition strengthens the rollup while shrinking the router's client base, producing a tipping dynamic that drives the \textbf{router to extinction across all 10 tested seeds}. The rollup achieves 3--4$\times$ the router's cumulative surplus.
...
\item The router is \textbf{worse than the market for leakage}: despite lower nominal visibility, the router's total leakage cost (0.72 per event) is $3\times$ the market's (0.23), because the router extracts and redistributes operational intelligence to competitors.
\end{enumerate}
...
Under the baseline calibration ($\lambda_L = 0.8$, $\gamma = 1.5$), match quality gains from revelation dominate leakage costs at every visibility level, so the optimizer selects $r = 1.0$ (full revelation) for all regimes in $\sim$99\% of events. The revelation-leakage tradeoff is therefore \textit{theoretical} under the baseline: it describes the correct comparative statics but does not produce regime-differentiated revelation behavior. At higher leakage intensity, the tradeoff binds and produces the predicted regime differentiation: at $\lambda_L = 5.0$, the rollup reveals fully in 85\% of events, the router in 63\%, and the market in 67\%---consistent with Proposition~\ref{prop:tradeoff}. We retain the baseline calibration because the ownership advantage does not depend on differentiated revelation; it is driven by router learning leakage, cross-node learning, and leakage protection, all of which operate at $r = 1.0$. A calibration where the tradeoff binds would add regime-differentiated revelation as an additional channel favoring the rollup. Under the baseline, the realized regime separation comes from router learning leakage, cross-node learning, and leakage protection---not from differentiated revelation behavior.
\end{remark}
...
We evaluate the model in two modes. In the \textbf{parallel-world comparison}, we run three separate simulations on the same graph---one where every node coordinates through the market (no firms, just agents), one where every node coordinates through the router (a single platform serves all clients---the router is not a node on the graph but an invisible institutional layer governing the rules of every interaction: how much information leaks, how trust builds, what the platform learns), and one where a rollup owns some nodes and the rest coordinate at arm's length. The three worlds never interact: the router does not lose clients when the rollup acquires nodes, because they exist in separate realities. Each world answers: ``if this were the \textit{only} institutional form available, how much surplus would the economy generate?'' This is useful for \textit{attribution}---isolating which channels create the structural advantage---because performance differences can only come from the institutional form, not from competitive dynamics.

In the \textbf{competing-world simulation} (Section~\ref{sec:competing}), all three forms coexist in a single economy. The rollup starts with 3 nodes, the router serves $\sim$14 clients, and the rest are market participants. Each time step, the rollup acquires one node---and that node \textit{actually leaves} the router's client base. This is useful for \textit{dynamics}---showing what happens when the structural advantage compounds through competition.

\subsection{What Happens in Each World: A Walkthrough}
...
\item \textbf{In the router world}, the same two nodes coordinate, but now their interaction flows through the platform's institutional layer. Visibility is slightly lower ($0.85$), trust builds faster through the platform's standardized interface (interaction mode $= 0.6$)---but the platform also extracts operational intelligence from the interaction (router learning leakage), imposing a cost that represents the value donated to the platform's shared model.

\item \textbf{In the rollup world}, if both nodes are owned, their interaction is internal: minimal leakage (visibility $= 0.1$), full operational control (actuator $= 1.0$), high-trust interaction (mode $= 1.0$), and no router learning leakage. If only one is owned, the interaction reverts to market-level parameters.
\end{itemize}

\noindent In every world, the same nodes coordinate over the same edges. What changes is the parameter regime governing each interaction. The ``router'' and ``rollup'' are not actors on the graph---they are the rules of the game, like playing the same position under different rulesets.

\subsection{Simulation Loop}

Each time step proceeds as follows, independently for each regime:

\begin{enumerate}[leftmargin=*]
\item \textbf{Context drift.} Every node's demand and capacity profiles receive a small random perturbation (shared across regimes, so all three face the same changing economy).

\item \textbf{Opportunity generation.} Each node independently generates a Poisson-distributed number of coordination opportunities (mean 0.3 per node per step). Across 30 nodes, this produces roughly 9 opportunities per step, each handled independently. A node may generate zero, one, or multiple opportunities in a single step.

\item \textbf{Counterparty selection and revelation.} For each opportunity, the node evaluates all its graph neighbors as potential coordination partners. For each candidate, the node must decide \textit{how much private context to reveal} --- the core tradeoff of the model. Revealing more context (higher $r$) improves match quality (better coordination outcome) but increases leakage cost (competitors can exploit the revealed information). The node evaluates five discrete revelation levels ($r \in \{0, 0.25, 0.5, 0.75, 1.0\}$, from ``reveal nothing'' to ``reveal everything'' --- a grid approximation rather than continuous optimization, since the net surplus function has no closed-form optimum) and picks the level that maximizes net surplus after accounting for match quality gains, leakage costs, and execution losses at the current state of trust, accumulated leakage stock, and policy quality. It then selects the single best counterparty --- the one producing the highest positive net surplus at its optimal revelation level. Each opportunity results in at most one coordination event. If no counterparty yields positive surplus at any revelation level, the opportunity goes unexercised.

\item \textbf{Coordination event.} If a counterparty is selected, the coordination occurs: match quality is computed, leakage costs are incurred, execution losses are realized, and net surplus is recorded.

\item \textbf{State updates.} After all events in the step: (a) pairwise trust is updated based on outcomes (success builds trust, failure erodes it); (b) leakage stocks accumulate and decay; (c) policy quality improves through learning (at a rate determined by signal quality and the transfer bonus); (d) the rollup pays ownership costs; (e) if the acquisition interval is reached, the rollup acquires the highest-EMV unowned node.
\end{enumerate}

\noindent After $T$ steps, the simulation produces per-regime totals for surplus, coordination count, average match quality, average leakage, and final policy quality.
...
\begin{table}[H]
\centering
\caption{Regime Comparison: Baseline (seed 42)}
\label{tab:baseline}
\begin{tabular}{lrrr}
\toprule
Metric & Market & Router & Rollup \\
\midrule
Gross coordination surplus & 1,792 & 1,558 & 2,138 \\
Ownership costs & 0 & 0 & 76 \\
\textbf{Net surplus} & \textbf{1,792} & \textbf{1,558} & \textbf{2,061} \\
Total coordinations & 599 & 592 & 638 \\
Avg match quality & 0.758 & 0.779 & 0.802 \\
Avg leakage cost & 0.227 & 0.716 & 0.094 \\
Final policy quality & 0.361 & 0.629 & 1.000 \\
Nodes owned & 0 & 0 & 12 \\
\bottomrule
\end{tabular}
\end{table}
...
On the baseline seed, the rollup generates 32\% higher net surplus than the router after deducting ownership costs. The most counter-intuitive result in the table is the leakage row: \textbf{the router's average leakage (0.716) is three times the market's (0.227), despite the router having lower nominal visibility} (0.85 vs.\ 1.0). A platform designed to intermediate coordination produces \textit{more} information leakage than the open market, not less. This inversion occurs because the router's learning loop systematically extracts operational intelligence from each client interaction and embeds it in a shared model that serves competitors---a cost absent from market interactions, where information is passively exposed but not actively harvested. Over 10 random graph realizations (seeds 42--51), the mean rollup/router ratio is \textbf{1.24} (std.\ 0.056), with the rollup generating higher net surplus in 10 of 10 seeds.
...
\midrule
Router learning leakage & Platform extracts/redistributes client data & $+0.208 \pm 0.009$ \\
Cross-node learning & Richer signal quality under ownership & $+0.067 \pm 0.011$ \\
Leakage protection & Lower information exposure for rollup & $+0.052 \pm 0.009$ \\
Actuator access & Rollup internal control advantage & $+0.005 \pm 0.002$ \\
Context foreclosure & Negligible under current calibration & $-0.004 \pm 0.001$ \\
Trust-generated context & Slightly favors the router & $-0.011 \pm 0.006$ \\
\midrule
\textbf{Total effect} & & $\mathbf{\sim +0.32}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Router learning leakage is the dominant channel} ($+0.208 \pm 0.009$), robust across all 5 seeds with the tightest standard deviation of any channel. If the router learning leakage hypothesis holds, it is the single largest driver of ownership advantage---accounting for roughly 65\% of the total effect.

\textbf{Cross-node learning} ($+0.067 \pm 0.011$) and \textbf{leakage protection} ($+0.052 \pm 0.009$) are stable secondary channels. Together with router learning leakage, these three per-interaction channels account for over 100\% of the total effect (the remaining channels are near zero or slightly negative).
...
\textbf{Trust slightly favors the router} ($-0.011 \pm 0.006$); \textbf{actuator is negligible} ($+0.005$). The rollup wins through information channels that the classical theory of the firm does not model.
...
The rollup/router ratio increases with ownership density, from below parity at 10\% (where ownership costs exceed the small information advantage) to 9.2\% at 90\%. This pattern is consistent with growing information asymmetry as more nodes are absorbed, but the multi-seed Shapley results (Table~\ref{tab:shapley}) show that foreclosure-via-context-degradation is not a stable positive contributor in parallel worlds ($-0.004 \pm 0.001$). The stronger role for foreclosure appears in the competing-world simulation (Section~\ref{sec:competing}), where acquisitions remove nodes from the router's client base entirely. The rollup consistently outperforms the market (Rollup/Market column) even at low density, because per-interaction channels (router learning leakage, cross-node learning, leakage protection) operate regardless of ownership density. All values are single-seed (seed 42).
...
\midrule
0.000 & 58 & 2,080 & 1,558 & 1.335 \\
0.050 & 76 & 2,061 & 1,558 & 1.323 \\
0.100 & 95 & 2,043 & 1,558 & 1.311 \\
0.200 & 133 & 2,005 & 1,558 & 1.287 \\
0.300 & 170 & 1,968 & 1,558 & 1.263 \\
\bottomrule
\end{tabular}
\end{table}

The rollup wins at every tested ownership cost level (single-seed). At the highest tested overhead (0.30 per node per step), the rollup still maintains a 26\% net advantage. The rollup's advantage is robust to ownership costs because the dominant channel (router learning leakage) does not depend on ownership cost---it is a cost the router bears regardless.

\subsection{Sensitivity to Router Learning Leakage}

The dominant Shapley channel ($\lambda_R = 0.15$) is an assumed parameter. How sensitive are the results to this assumption?

\begin{table}[H]
\centering
\caption{Rollup/Router ratio vs.\ router learning leakage rate (5 seeds each)}
...
\noindent At $\lambda_R = 0$, the rollup still wins (ratio 1.035) through the other channels---learning, leakage protection, and foreclosure---but the advantage is slim. Each increment of $\lambda_R$ widens the gap substantially. The results are monotonic but not fragile: the rollup wins at every tested value, and the qualitative conclusion (ownership dominates intermediation) holds even without router learning leakage. The quantitative magnitude, however, depends materially on how aggressively platforms extract and redistribute client data.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_tipping.pdf}
\caption{Competing-world surplus trajectory (10 seeds, mean $\pm$ 1 s.d., passive router). The rollup starts near zero (paying integration costs), overtakes the router at step 18--30 (mean 24) across all seeds, and grows to $\sim$30 per step while the router goes to zero. Shaded bands show cross-seed variability. The market persists at a stable level because its low-value nodes are not worth acquiring.}
\label{fig:tipping}
\end{figure}
...
\textbf{What is the tipping point?} The crossover occurs when the rollup's per-step surplus first exceeds the router's (step 18--30, mean 24, across 10 seeds). At this point, the rollup has acquired enough nodes that two reinforcing effects dominate: (1) its internal interactions---which have lower leakage, richer learning signal, and no router learning leakage cost---generate more surplus than the router's shrinking client base, and (2) the router's aggregate learning breadth has degraded enough (from losing clients) that its policy quality improvement stalls while the rollup's continues to accelerate.

The tipping point does not admit a closed-form expression because it depends on the interaction of multiple graph-dependent quantities: the topology of edges between owned and unowned nodes, the distribution of hidden alpha across acquired vs.\ remaining nodes, the accumulated trust and leak stock states, and the specific sequence of acquisitions. Each acquisition changes all of these simultaneously, creating path dependence that prevents reduction to a single threshold formula. What can be said analytically is that the tipping becomes inevitable once the rollup's ownership density is high enough that its internal surplus per step exceeds the router's total---after which every further acquisition widens the gap.

The parallel and competing results reinforce each other. The parallel model identifies \textit{which channels} create the structural edge (router learning leakage, cross-node learning, leakage protection). The competing model shows \textit{what happens when that edge compounds}: not a permanent marginal advantage, but a tipping dynamic that eliminates the intermediary.
...
The Shapley decomposition and $\lambda_R$ sensitivity analysis identify the boundary conditions:

\begin{enumerate}[leftmargin=*]
\item \textbf{If router learning leakage is absent} ($\lambda_R = 0$), the rollup still wins but by only 3.5\% (Table~\ref{tab:lr_sensitivity}). The thesis depends materially on the hypothesis that platforms extract and redistribute client data. In industries where platforms contractually commit to data isolation (and enforce it), this channel may not operate.

\item \textbf{Without cross-node learning transfer} (Shapley $+0.065$), the rollup loses its second-largest channel. If operations are too heterogeneous for knowledge to transfer across portfolio companies, or if AI cannot extract transferable patterns, the advantage narrows further.

\item \textbf{If leakage is unimportant}---regulated industries with mandatory disclosure, standardized commodity markets, or contexts where information is too short-lived to exploit---the leakage protection channel ($+0.039$) contributes nothing. In most economically interesting coordination, however, context is private, persistent, and valuable.
\end{enumerate}

\noindent These conditions suggest the rollup thesis applies most strongly to industries where: (a) SaaS platforms learn from client data in ways that benefit competitors, (b) operations are diverse enough for cross-vertical knowledge transfer, and (c) private context has high competitive value.
